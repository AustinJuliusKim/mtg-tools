...
export function History({ revision }: { revision: number }) {
  ...
  useEffect(() => { api.history().then(setRows).catch(() => undefined) }, [revision])
    ...
    <>
      ...
      <Card withBorder padding={0} radius="md">
        <Table>
          <Table.Thead>
            <Table.Tr>
              {['When', 'Change', 'Rows', 'State'].map((h) => <Table.Th key={h}>{h}</Table.Th>)}
            </Table.Tr>
          </Table.Thead>
          <Table.Tbody>
            {rows.map((op) => (
              <Table.Tr key={op.id} opacity={op.reverted ? 0.55 : 1}>
                <Table.Td c="dimmed" fz="xs">{op.createdAt.slice(0, 16).replace('T', ' ')}</Table.Td>
        ...
        {!rows.length && <Text c="dimmed" ta="center" p="xl">Nothing has changed yet.</Text>}
      </Card>
    </>
  )
}
